<template>
            <div class="wave-echart" id='waveEchart' ref='waveEchart' style="width:140px;height:140px" ></div>
</template>
<script>
...
export default {
  ...
  methods: {
    initWave (ref) {
      logisticsSituationAPI.getTaskProgress({}).then((res) => {
        this.completionRate = res.m.normalRate;
        this.glData = this.completionRate/100;
        let chart = echarts.init(document.getElementById("waveEchart"));
        // 把配置和数据放这里
        let option={
          title: {// 标题
            // text: '补贴资金剩余',
