<template>
    <div class="left-chart-one-component">
...
export default {
    ...
    mounted() {
        ...
        // })
        this.$eventBus.$on('left-chart-one-refresh', () => {
            this.fetchDatas()
        })
    },
    beforeDestroy() {
        this.$eventBus.$off('left-chart-one-refresh')
    },
    methods: {
        fetchDatas() {
            HomeAPI.fetchLeftInStoreTimeDistribution({}).then(res => {
        ...
        renderChart(echartRef, datas = []) {
            let _titles = ['0-4', '4-8', '8-12', '12-16', '16-20', '20-24', '24小时以上'];
            let _datas = [ 0, 0, 0, 0, 0, 0, 0];
            datas.map(r => {
                let _pos = _titles.indexOf(r.time_distribution);
                if (_pos > -1) {
                    _datas[_pos] = r.total_num;
                }
            })
            let _option = {
                legend: {
                    data: [ {
                        name: '入库时长分布', icon: 'path://M0 2a2 2 0 0 1 2 -2h14a2 2 0 0 1 2 2v0a2 2 0 0 1 -2 2h-14a2 2 0 0 1 -2 -2z'
                    } ],
