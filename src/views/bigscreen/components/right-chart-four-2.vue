...
export default {
    ...
    mounted() {
        this.fetchDatas();
        this.$eventBus.$on('right-chart-four2-refresh', () => {
            this.fetchDatas()
        })
    },
    beforeDestroy() {
        this.$eventBus.$off('right-chart-four2-refresh')
    },
    methods: {
        fetchDatas() {
            this.refChart = Echarts.init(this.$refs["block-chart"]);
            HomeAPI.fetchRightUnsalableRate({}).then(res => {
