...
//添加5类点位
export function pointLayer(EtsMaps, extrudedHeights, optionData) {
    EtsMap = EtsMaps;
...
function addIcon(param) {
    ...
    var item = {
        id: param.id ? param.id : obj.options.id,
...
//图层显示隐藏
export function controleLayer(option) {
    $("#moveTips").hide();
    removeHrightLight();
    layers.forEach((element) => {
        if (element.type === option.type) {
            if (option.isOpen) {
                element.markerLayer.show();
            } else {
                element.markerLayer.hide();
            }
        }
    });
}
export function hideLayer(visible) {
    $("#moveTips").hide();
    removeHrightLight();
    layers.forEach((element) => {
        if (visible) {
...
export function PointHeightLight(id) {
    ...
    if (id) {
        layers.forEach((element) => {
            marker = element.markerLayer.get(id);
            if (marker) {
                ...
                const param = {
                    id: id + "heightlight",
                    old: id,
                    oldMarkerLayer: element.markerLayer,
