import { pointLayer, controleLayer, PointHeightLight } from '../map3d/pointlayer'
import { searchPointLayer } from '../map3d/searchLayer'
export default class MapLayer {
    ...
    init() {
        ...
        this.provinceMaterial = this.getColorRamp([0.0, 0.045, 0.1, 0.15, 0.37, 0.54, 1.0],true);
        this.SelectedProvinceMaterial = this.getselectColorRamp([0.0, 0.045, 0.1, 0.15, 0.37, 0.54, 1.0],true);
    ...
    getColorRamp(elevationRamp) {
        ...
        grd.addColorStop(values[0], '#02236e'); //深蓝
        grd.addColorStop(values[1], '#032981'); 
        grd.addColorStop(values[2], '#022f99'); 
        grd.addColorStop(values[3], '#0334a7'); 
        grd.addColorStop(values[4], '#0339b7'); 
        grd.addColorStop(values[5], '#0a41c3'); 
        grd.addColorStop(values[6], '#3b71f3'); //浅蓝

        ctx.fillStyle = grd;
        ctx.fillRect(0, 0, 1, 100);
        return ramp;
    }
    getselectColorRamp(elevationRamp) {
        var ramp = document.createElement('canvas');
        ramp.width = 1;
        ramp.height = 100;
        var ctx = ramp.getContext('2d');
        ...
        grd.addColorStop(values[2], '#022f99'); 
        grd.addColorStop(values[3], '#0334a7'); 
        grd.addColorStop(values[4], '#3daef9'); //black
        grd.addColorStop(values[5], '#4eb6fb'); //pink
        grd.addColorStop(values[6], '#01ffff'); //white
        ctx.fillStyle = grd;
        ctx.fillRect(0, 0, 1, 100);
        return ramp;
    }
    ...
    addODLine(mapData) {
        this.wanchengData = this.dealData(mapData.line2)
        this.weiwancheng = this.dealData(mapData.line1)
    ...
    loadChinaBounds(etsMap) {
        ...
        promise.then(function (dataSource) {
            ...
            for (let i = 0; i < entities.length; i++) {
                ...
                entities[i].polygon.outlineWidth = 20;
    ...
    loadChina(etsMap) {
        ...
        viewer.camera.setView({
            ...
            destination: etsMap._etsMap.Cartesian3.fromDegrees(112.84, 5.16, 5000000),
            orientation: {
